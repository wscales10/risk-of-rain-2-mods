...
namespace Rules.RuleTypes.Mutable
{
    public class MultiRule<T, TContext, TOut> : UpperRule<TContext, TOut>, IMultiRule<T, TContext, TOut>
    {
        ...
        {
            return (ArrayRule<TContext, TOut>)new ArrayRule<TContext, TOut>(mr.Pairs.Select((pair) => new IfRule<TContext, TOut>(PropertyPattern<TContext>.Create(mr.PropertyName, mr.patternGenerator(pair.expectedValue)), pair.rule)).ToArray()).Named(mr.Name);
        }
