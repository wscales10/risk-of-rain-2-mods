...
namespace RuleExamples.RiskOfRain2
{
    ...
    public static class Ror2Rule
    {
        private static readonly Rule<RoR2Context, string> IdleRule = Switcher.RoR2ToString.Create(nameof(RoR2Context.SceneName), ScenePattern.Equals,
                    C<MyScene>("Abandoned Aqueduct (Simulacrum)", Scenes.AbandonedAqueduct).Where(Query.Create<RunType>(nameof(RoR2Context.RunType), EnumRangePattern.Equals(RunType.Simulacrum))),
        ...
        private static readonly Rule<RoR2Context, string> OtherRule = Switcher.RoR2ToString.Create(
            ...
            C<MyScene>("Normal Character Select", Scenes.CharacterSelect).Where(Query.Create<RunType>(nameof(RoR2Context.RunType), EnumRangePattern.Equals(RunType.Normal))),
            C<MyScene>("Eclipse Menus", Scenes.EclipseMenu, Scenes.CharacterSelect).Where(Query.Create<RunType>(nameof(RoR2Context.RunType), EnumRangePattern.Equals(RunType.Eclipse))),
            C<MyScene>("Prismatic Trials Menus", Scenes.PrismaticTrialsMenu, Scenes.CharacterSelect).Where(Query.Create<RunType>(nameof(RoR2Context.RunType), EnumRangePattern.Equals(RunType.PrismaticTrial))),
            C<MyScene>("Simulacrum Menus", Scenes.SimulacrumMenu, Scenes.CharacterSelect).Where(Query.Create<RunType>(nameof(RoR2Context.RunType), EnumRangePattern.Equals(RunType.Simulacrum))));
