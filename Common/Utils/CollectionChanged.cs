	...
	public static class CollectionChanged
	{
		...
		private static List<T> getOldValues<T>(object sender, NotifyCollectionChangedEventArgs e)
		{
			...
			switch (e.Action)
			{
				...
				case NotifyCollectionChangedAction.Move:
					newValues.RemoveRange(e.NewStartingIndex, e.NewItems.Count);
					newValues.InsertRange(e.OldStartingIndex, (IEnumerable<T>)e.OldItems);
					break;
				...
				case NotifyCollectionChangedAction.Replace:
					newValues.ReplaceRange(e.NewStartingIndex, (IEnumerable<T>)e.OldItems);
					break;

				case NotifyCollectionChangedAction.Reset:
					newValues.AddRange((IEnumerable<T>)e.OldItems);
					break;
			}
