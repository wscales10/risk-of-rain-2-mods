...
namespace Patterns.Patterns
{
	...
	{
		private static readonly Cache<Type, LazyList<PropertyInfo>> cache = new Cache<Type, LazyList<PropertyInfo>>
		(
			t => LazyList.Create
			(
				new[] { new PropertyInfo("this", t) }.Concat(t.GetPublicProperties().Select(p => new PropertyInfo(p.Name, p.PropertyType)))
			)
		);
